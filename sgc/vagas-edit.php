...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_vagas SET POS_COD_HOME=%s, POS_COD_CANAL=%s, VAG_TITULO=%s, VAG_CORPO=%s, VAG_STATUS=%s, VAG_DATA_DIA=%s, VAG_DATA_MES=%s, VAG_DATA_ANO=%s, VAG_QUANTIDADE=%s WHERE VAG_COD=%s",
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['VAG_TITULO'], "text"),
                       GetSQLValueString($_POST['VAG_CORPO'], "text"),
                       GetSQLValueString($_POST['VAG_STATUS'], "int"),
                       GetSQLValueString($_POST['VAG_DATA_DIA'], "int"),
                       GetSQLValueString($_POST['VAG_DATA_MES'], "int"),
                       GetSQLValueString($_POST['VAG_DATA_ANO'], "int"),
                       GetSQLValueString($_POST['VAG_QUANTIDADE'], "int"),
                       GetSQLValueString($_POST['VAG_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "vagas.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsVAGAS = "-1";
if (isset($_GET['URL_VAG_COD'])) {
  $colname_rsVAGAS = $_GET['URL_VAG_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsVAGAS = sprintf("SELECT * FROM tb_vagas WHERE VAG_COD = %s", GetSQLValueString($colname_rsVAGAS, "int"));
...
<title>Editar &laquo; Vagas &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="VAG_COD" type="hidden" id="VAG_COD" value="<?php echo $row_rsVAGAS['VAG_COD']; ?>" />
                  <div class="formRight"><input name="VAG_TITULO" type="text" id="VAG_TITULO" value="<?php echo $row_rsVAGAS['VAG_TITULO']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem">
                        <label>Data:</label> 
                        <div class="formRight moreFields onlyNums">
                            <ul>
                                <li><input type="text" name="VAG_DATA_DIA" maxlength="2" value=<?php echo $row_rsVAGAS['VAG_DATA_DIA']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="VAG_DATA_MES" maxlength="2" value=<?php echo $row_rsVAGAS['VAG_DATA_MES']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="VAG_DATA_ANO" maxlength="4" value=<?php echo $row_rsVAGAS['VAG_DATA_ANO']; ?> /></li>
                  ...
                  <div class="formRight"><input name="VAG_QUANTIDADE" type="text" id="VAG_QUANTIDADE" value="<?php echo $row_rsVAGAS['VAG_QUANTIDADE']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem nobg">
                      <label>Status</label>
                        <div class="formRight">
                        <select name="VAG_STATUS" id="VAG_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsVAGAS['VAG_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsVAGAS['VAG_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsVAGAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsVAGAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsVAGAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsVAGAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                ...
                <div class="formRight"><textarea name="VAG_CORPO" cols="" rows="8" id="VAG_CORPO"><?php echo $row_rsVAGAS['VAG_CORPO']; ?></textarea></div><div class="fix"></div></div>
