...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_depoimentos SET DEP_TITULO=%s, DEP_CLIENTE=%s, DEP_CORPO=%s, DEP_STATUS=%s WHERE DEP_COD=%s",
                       GetSQLValueString($_POST['DEP_TITULO'], "text"),
                       GetSQLValueString($_POST['DEP_CLIENTE'], "text"),
                       GetSQLValueString($_POST['DEP_CORPO'], "text"),
                       GetSQLValueString($_POST['DEP_STATUS'], "int"),
                       GetSQLValueString($_POST['DEP_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "depoimentos.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsFrases = "-1";
if (isset($_GET['URL_DEP_COD'])) {
  $colname_rsFrases = $_GET['URL_DEP_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsFrases = sprintf("SELECT * FROM tb_depoimentos WHERE DEP_COD = %s ORDER BY DEP_COD DESC", GetSQLValueString($colname_rsFrases, "int"));
      ...
      <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Autor:</label>
                  <input name="DEP_COD" type="hidden" id="DEP_COD" value="<?php echo $row_rsFrases['DEP_COD']; ?>" />
                  <div class="formRight"><input name="DEP_CLIENTE" type="text" id="DEP_CLIENTE" value="<?php echo $row_rsFrases['DEP_CLIENTE']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem nobg">
                  <label>Título:</label>
                  <div class="formRight"><input name="DEP_TITULO" type="text" id="DEP_TITULO" value="<?php echo $row_rsFrases['DEP_TITULO']; ?>"/></div><div class="fix"></div></div>
                        ...
                        <select name="DEP_STATUS" id="DEP_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsFrases['DEP_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsFrases['DEP_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  <div class="rowElem">
                  <label>Frase:</label><div class="formRight"><textarea name="DEP_CORPO" cols="" rows="8" id="DEP_CORPO"><?php echo $row_rsFrases['DEP_CORPO']; ?></textarea>
