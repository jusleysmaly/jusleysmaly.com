...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_clientes SET CLI_NOME=%s, CLI_RAMO=%s, CLI_TAGS=%s, CLI_STATUS=%s, CLI_URL=%s WHERE CLI_COD=%s",
                       GetSQLValueString($_POST['CLI_NOME'], "text"),
                       GetSQLValueString($_POST['CLI_RAMO'], "text"),
                       GetSQLValueString($_POST['CLI_TAGS'], "text"),
                       GetSQLValueString($_POST['CLI_STATUS'], "int"),
                       GetSQLValueString($_POST['CLI_URL'], "text"),
...
if (isset($_GET['URL_CLI_COD'])) {
  $colname_rsCLIENTES = $_GET['URL_CLI_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsCLIENTES = sprintf("SELECT * FROM tb_clientes WHERE CLI_COD = %s", GetSQLValueString($colname_rsCLIENTES, "int"));
...
<title>Editar &laquo; Clientes &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Nome:</label>
                  <input name="CLI_COD" type="hidden" id="CLI_COD" value="<?php echo $row_rsCLIENTES['CLI_COD']; ?>" />
                  <div class="formRight"><input name="CLI_NOME" type="text" id="CLI_NOME" value="<?php echo $row_rsCLIENTES['CLI_NOME']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem nobg">
                  <label>Ramo:</label>
                  
                  <div class="formRight"><input name="CLI_RAMO" type="text" id="CLI_RAMO" value="<?php echo $row_rsCLIENTES['CLI_RAMO']; ?>"/></div><div class="fix"></div></div>
                  
                  
                  <div class="rowElem nobg">
                      <label>Status</label>
                        <div class="formRight">
                        <select name="CLI_STATUS" id="CLI_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsCLIENTES['CLI_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsCLIENTES['CLI_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                    <label>Website:</label>
                <div class="formRight">
                  <input name="CLI_URL" type="text" id="CLI_URL" value="<?php echo $row_rsCLIENTES['CLI_URL']; ?>" size="" />
                </div><div class="fix"></div></div>
                
                  <div class="rowElem">
                    <label>Tags:</label>
                <div class="formRight">
                  <input name="CLI_TAGS" type="text" id="CLI_TAGS" value="<?php echo $row_rsCLIENTES['CLI_TAGS']; ?>" size="" />
                </div><div class="fix"></div></div>
