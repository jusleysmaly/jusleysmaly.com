...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_publicidade SET POS_COD_HOME=%s, POS_COD_CANAL=%s, PUB_TITULO=%s, PUB_LINK=%s, PUB_STATUS=%s, PUB_DESC=%s, PUB_TIPO=%s WHERE PUB_COD=%s",
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['PUB_TITULO'], "text"),
                       GetSQLValueString($_POST['PUB_LINK'], "text"),
                       GetSQLValueString($_POST['PUB_STATUS'], "int"),
                       GetSQLValueString($_POST['PUB_DESC'], "text"),
                       GetSQLValueString($_POST['PUB_TIPO'], "int"),
                       GetSQLValueString($_POST['PUB_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "publicidade.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsPUBLICIDADE = "-1";
if (isset($_GET['URL_PUB_COD'])) {
  $colname_rsPUBLICIDADE = $_GET['URL_PUB_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsPUBLICIDADE = sprintf("SELECT * FROM tb_publicidade WHERE PUB_COD = %s", GetSQLValueString($colname_rsPUBLICIDADE, "int"));
$rsPUBLICIDADE = mysql_query($query_rsPUBLICIDADE, $conn92ID) or die(mysql_error());
$row_rsPUBLICIDADE = mysql_fetch_assoc($rsPUBLICIDADE);
$totalRows_rsPUBLICIDADE = mysql_num_rows($rsPUBLICIDADE);
...
<title>Inserir Novo &laquo; Publicidade &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
...
<div class="wrapper">
    ...
    <div class="content" id="container">
    	<div class="title">
    	  <h5>Publicidade &laquo; Editar</h5></div>
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="PUB_COD" type="hidden" id="PUB_COD" value="<?php echo $row_rsPUBLICIDADE['PUB_COD']; ?>" />
                  <div class="formRight"><input name="PUB_TITULO" type="text" id="PUB_TITULO" value="<?php echo $row_rsPUBLICIDADE['PUB_TITULO']; ?>"/></div><div class="fix"></div></div>
                        
                        
                        <div class="rowElem">
                    <label>URL:</label>
                    <div class="formRight"><input name="PUB_LINK" type="text" id="PUB_LINK" value="<?php echo $row_rsPUBLICIDADE['PUB_LINK']; ?>"/></div><div class="fix"></div></div>
                    
                    <div class="rowElem">
                    <label>Descri&ccedil;&atilde;o:</label>
                    <div class="formRight"><input name="PUB_DESC" type="text" id="PUB_DESC" value="<?php echo $row_rsPUBLICIDADE['PUB_DESC']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem">
                        <label>Tipo:</label>
                        <div class="formRight">
                        <select name="PUB_TIPO" id="PUB_TIPO" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPUBLICIDADE['PUB_TIPO']))) {echo "selected=\"selected\"";} ?>>Banner</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPUBLICIDADE['PUB_TIPO']))) {echo "selected=\"selected\"";} ?>>Link</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                        ...
                        <select name="PUB_STATUS" id="PUB_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPUBLICIDADE['PUB_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPUBLICIDADE['PUB_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  <div class="rowElem">
                    <label>Posição / Home</label>
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0"  <?php if (!(strcmp(0, $row_rsPUBLICIDADE['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPUBLICIDADE['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPUBLICIDADE['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPUBLICIDADE['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
