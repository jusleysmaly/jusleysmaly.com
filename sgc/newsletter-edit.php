...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_newsletter SET NEW_TITULO=%s, NEW_CLIENTE=%s, NEW_CORPO=%s, NEW_STATUS=%s, NEW_ADS_TITULO=%s, NEW_ADS_CORPO=%s, NEW_ADS_LINK=%s WHERE NEW_COD=%s",
                       GetSQLValueString($_POST['NEW_TITULO'], "text"),
                       GetSQLValueString($_POST['NEW_CLIENTE'], "text"),
                       GetSQLValueString($_POST['NEW_CORPO'], "text"),
                       GetSQLValueString($_POST['NEW_STATUS'], "int"),
                       GetSQLValueString($_POST['NEW_ADS_TITULO'], "text"),
                       GetSQLValueString($_POST['NEW_ADS_CORPO'], "text"),
                       GetSQLValueString($_POST['NEW_ADS_LINK'], "text"),
                       GetSQLValueString($_POST['NEW_COD'], "int"));
...
if (isset($_GET['URL_NEW_COD'])) {
  $colname_rsFrases = $_GET['URL_NEW_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsFrases = sprintf("SELECT * FROM tb_newsletter WHERE NEW_COD = %s ORDER BY NEW_COD DESC", GetSQLValueString($colname_rsFrases, "int"));
      ...
      <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Link:</label>
                  <input name="NEW_COD" type="hidden" id="NEW_COD" value="<?php echo $row_rsFrases['NEW_COD']; ?>" />
                  <div class="formRight"><input name="NEW_CLIENTE" type="text" id="NEW_CLIENTE" value="<?php echo $row_rsFrases['NEW_CLIENTE']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem nobg">
                  <label>Titulo:</label>
                  <div class="formRight"><input name="NEW_TITULO" type="text" id="NEW_TITULO" value="<?php echo $row_rsFrases['NEW_TITULO']; ?>"/></div><div class="fix"></div></div>
                        ...
                        <select name="NEW_STATUS" id="NEW_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsFrases['NEW_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsFrases['NEW_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  <div class="rowElem">
                  <label>Resumo:</label><div class="formRight"><textarea name="NEW_CORPO" cols="" rows="8" id="NEW_CORPO"><?php echo $row_rsFrases['NEW_CORPO']; ?></textarea>
                  </div><div class="fix"></div></div>
                  
                   <!-- Ads -->
                  
                 <div class="rowElem">
                  <label>Ads Titulo:</label>
                  <div class="formRight"><input name="NEW_ADS_TITULO" type="text" id="NEW_ADS_TITULO" value="<?php echo $row_rsFrases['NEW_ADS_TITULO']; ?>"/></div><div class="fix"></div></div>
                  
                   <div class="rowElem">
                  <label>Ads Link:</label>
                  <div class="formRight"><input name="NEW_ADS_LINK" type="text" id="NEW_ADS_LINK"/></div><div class="fix"></div></div>
                    
                  <div class="rowElem">
                  <label>Ads Texto:</label><div class="formRight"><textarea name="NEW_ADS_CORPO" cols="" rows="8" id="NEW_ADS_CORPO"><?php echo $row_rsFrases['NEW_ADS_CORPO']; ?></textarea></div><div class="fix"></div></div>
