...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_parceiros SET PAR_NOME=%s, PAR_RAMO=%s, PAR_TAGS=%s, PAR_STATUS=%s, PAR_URL=%s WHERE PAR_COD=%s",
                       GetSQLValueString($_POST['PAR_NOME'], "text"),
                       GetSQLValueString($_POST['PAR_RAMO'], "text"),
                       GetSQLValueString($_POST['PAR_TAGS'], "text"),
                       GetSQLValueString($_POST['PAR_STATUS'], "int"),
                       GetSQLValueString($_POST['PAR_URL'], "text"),
                       GetSQLValueString($_POST['PAR_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "parceiros.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['URL_PAR_COD'])) {
  $colname_rsPARCEIROS = $_GET['URL_PAR_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsPARCEIROS = sprintf("SELECT * FROM tb_parceiros WHERE PAR_COD = %s", GetSQLValueString($colname_rsPARCEIROS, "int"));
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Nome:</label>
                  <input name="PAR_COD" type="hidden" id="PAR_COD" value="<?php echo $row_rsPARCEIROS['PAR_COD']; ?>" />
                  <div class="formRight"><input name="PAR_NOME" type="text" id="PAR_NOME" value="<?php echo $row_rsPARCEIROS['PAR_NOME']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem nobg">
                  <label>Ramo:</label>
                  
                  <div class="formRight"><input name="PAR_RAMO" type="text" id="PAR_RAMO" value="<?php echo $row_rsPARCEIROS['PAR_RAMO']; ?>"/></div><div class="fix"></div></div>
                  
                  
                  <div class="rowElem nobg">
                      <label>Status</label>
                        <div class="formRight">
                        <select name="PAR_STATUS" id="PAR_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPARCEIROS['PAR_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPARCEIROS['PAR_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                    <label>Website:</label>
                <div class="formRight">
                  <input name="PAR_URL" type="text" id="PAR_URL" value="<?php echo $row_rsPARCEIROS['PAR_URL']; ?>" size="" />
                </div><div class="fix"></div></div>
                
                  <div class="rowElem">
                    <label>Tags:</label>
                <div class="formRight">
                  <input name="PAR_TAGS" type="text" id="PAR_TAGS" value="<?php echo $row_rsPARCEIROS['PAR_TAGS']; ?>" size="" />
                </div><div class="fix"></div></div>
