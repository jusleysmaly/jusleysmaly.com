...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_produtos_servicos SET POS_COD_HOME=%s, POS_COD_CANAL=%s, PRD_TITULO=%s, PRD_CORPO=%s, PRD_CORPO_HOME=%s, PRD_FOTO_LEGENDA=%s, PRD_STATUS=%s, PRD_TAGS=%s, PRDT_COD=%s WHERE PRD_COD=%s",
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['PRD_TITULO'], "text"),
                       GetSQLValueString($_POST['PRD_CORPO'], "text"),
                       GetSQLValueString($_POST['PRD_CORPO_HOME'], "text"),
                       GetSQLValueString($_POST['PRD_FOTO_LEGENDA'], "text"),
                       GetSQLValueString($_POST['PRD_STATUS'], "int"),
                       GetSQLValueString($_POST['PRD_TAGS'], "text"),
                       GetSQLValueString($_POST['PRDT_COD'], "int"),
                       GetSQLValueString($_POST['PRD_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "produtos-servicos.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsPRODUTOS_SERVICOS_TIPO_TIPO = "SELECT * FROM tb_produtos_servicos_tipo";
$rsPRODUTOS_SERVICOS_TIPO_TIPO = mysql_query($query_rsPRODUTOS_SERVICOS_TIPO_TIPO, $conn92ID) or die(mysql_error());
$row_rsPRODUTOS_SERVICOS_TIPO_TIPO = mysql_fetch_assoc($rsPRODUTOS_SERVICOS_TIPO_TIPO);
$totalRows_rsPRODUTOS_SERVICOS_TIPO_TIPO = mysql_num_rows($rsPRODUTOS_SERVICOS_TIPO_TIPO);

$colname_rsPRODUTOS_SERVICOS = "-1";
if (isset($_GET['URL_PRD_COD'])) {
  $colname_rsPRODUTOS_SERVICOS = $_GET['URL_PRD_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsPRODUTOS_SERVICOS = sprintf("SELECT * FROM tb_produtos_servicos WHERE PRD_COD = %s", GetSQLValueString($colname_rsPRODUTOS_SERVICOS, "int"));
...
<title>Produtos e Servi&ccedil;os &laquo; 92ID:CMS &bull; &copy;92dpi</title>
<!-- InstanceEndEditable -->
...
<div class="wrapper">
    ...
    <div class="content" id="container">
    	<div class="title">
    	  <h5>Produtos e Servi&ccedil;os</h5></div>
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="PRD_COD" type="hidden" id="PRD_COD" value="<?php echo $row_rsPRODUTOS_SERVICOS['PRD_COD']; ?>" />
                  <div class="formRight"><input name="PRD_TITULO" type="text" id="PRD_TITULO" value="<?php echo $row_rsPRODUTOS_SERVICOS['PRD_TITULO']; ?>"/></div><div class="fix"></div></div>
                        
                        
                        <div class="rowElem nobg">
                        <label>Categoria</label>
                        <div class="formRight">
                        <select name="PRDT_COD" id="PRDT_COD" >
                          <?php
do {  
?>
                          <option value="<?php echo $row_rsPRODUTOS_SERVICOS_TIPO_TIPO['PRDT_COD']?>"<?php if (!(strcmp($row_rsPRODUTOS_SERVICOS_TIPO_TIPO['PRDT_COD'], $row_rsPRODUTOS_SERVICOS['PRDT_COD']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsPRODUTOS_SERVICOS_TIPO_TIPO['PRDT_NOME']?></option>
                          <?php
} while ($row_rsPRODUTOS_SERVICOS_TIPO_TIPO = mysql_fetch_assoc($rsPRODUTOS_SERVICOS_TIPO_TIPO));
  $rows = mysql_num_rows($rsPRODUTOS_SERVICOS_TIPO_TIPO);
  if($rows > 0) {
      mysql_data_seek($rsPRODUTOS_SERVICOS_TIPO_TIPO, 0);
	  $row_rsPRODUTOS_SERVICOS_TIPO_TIPO = mysql_fetch_assoc($rsPRODUTOS_SERVICOS_TIPO_TIPO);
  }
?>
                        </select>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="PRD_STATUS" id="PRD_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPRODUTOS_SERVICOS['PRD_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPRODUTOS_SERVICOS['PRD_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    <div class="rowElem nobg">
                      <label>Posição / Home</label>
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0"  <?php if (!(strcmp(0, $row_rsPRODUTOS_SERVICOS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPRODUTOS_SERVICOS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsPRODUTOS_SERVICOS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsPRODUTOS_SERVICOS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                    ...
                    <div class="formRight"><input name="PRD_FOTO_LEGENDA" type="text" id="PRD_FOTO_LEGENDA" value="<?php echo $row_rsPRODUTOS_SERVICOS['PRD_FOTO_LEGENDA']; ?>" /></div><div class="fix"></div></div>
                  <div class="rowElem">
                  <label>Tags:</label><div class="formRight"><input name="PRD_TAGS" type="text" id="PRD_TAGS" value="<?php echo $row_rsPRODUTOS_SERVICOS['PRD_TAGS']; ?>" /></div><div class="fix"></div></div>
              
                        <div class="rowElem">
                          <label>Resumo:</label><div class="formRight"><textarea name="PRD_CORPO_HOME" cols="" rows="8" id="PRD_CORPO_HOME"><?php echo $row_rsPRODUTOS_SERVICOS['PRD_CORPO_HOME']; ?></textarea>
                  </div><div class="fix"></div></div>
                        <div class="rowElem">
                        <label>Descri&ccedil;&atilde;o completa:</label><div class="formRight"><textarea name="PRD_CORPO" cols="" rows="5" class="wysiwyg" id="PRD_CORPO"><?php echo $row_rsPRODUTOS_SERVICOS['PRD_CORPO']; ?></textarea>
                  </div><div class="fix"></div></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsPRODUTOS_SERVICOS_TIPO_TIPO);
