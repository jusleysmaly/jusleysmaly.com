...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_dicas SET POS_COD_HOME=%s, POS_COD_CANAL=%s, DIC_TITULO=%s, DIC_CORPO=%s, DIC_CORPO_HOME=%s, DIC_FOTO_LEGENDA=%s, DIC_STATUS=%s, DIC_FONTE=%s, DIC_TAGS=%s, DIC_DATA_DIA=%s, DIC_DATA_MES=%s, DIC_DATA_ANO=%s, DICT_COD=%s WHERE DIC_COD=%s",
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['DIC_TITULO'], "text"),
                       GetSQLValueString($_POST['DIC_CORPO'], "text"),
                       GetSQLValueString($_POST['DIC_CORPO_HOME'], "text"),
                       GetSQLValueString($_POST['DIC_FOTO_LEGENDA'], "text"),
                       GetSQLValueString($_POST['DIC_STATUS'], "int"),
                       GetSQLValueString($_POST['DIC_FONTE'], "text"),
                       GetSQLValueString($_POST['DIC_TAGS'], "text"),
                       GetSQLValueString($_POST['DIC_DATA_DIA'], "int"),
                       GetSQLValueString($_POST['DIC_DATA_MES'], "int"),
                       GetSQLValueString($_POST['DIC_DATA_ANO'], "int"),
                       GetSQLValueString($_POST['DICT_COD'], "int"),
                       GetSQLValueString($_POST['DIC_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "dicas.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsDICAS_TIPO = "SELECT * FROM tb_dicas_tipo";
$rsDICAS_TIPO = mysql_query($query_rsDICAS_TIPO, $conn92ID) or die(mysql_error());
$row_rsDICAS_TIPO = mysql_fetch_assoc($rsDICAS_TIPO);
$totalRows_rsDICAS_TIPO = mysql_num_rows($rsDICAS_TIPO);

$colname_rsDICAS = "-1";
if (isset($_GET['URL_DIC_COD'])) {
  $colname_rsDICAS = $_GET['URL_DIC_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsDICAS = sprintf("SELECT * FROM tb_dicas WHERE DIC_COD = %s", GetSQLValueString($colname_rsDICAS, "int"));
$rsDICAS = mysql_query($query_rsDICAS, $conn92ID) or die(mysql_error());
$row_rsDICAS = mysql_fetch_assoc($rsDICAS);
$totalRows_rsDICAS = mysql_num_rows($rsDICAS);
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="DIC_COD" type="hidden" id="DIC_COD" value="<?php echo $row_rsDICAS['DIC_COD']; ?>" />
                  <div class="formRight"><input name="DIC_TITULO" type="text" id="DIC_TITULO" value="<?php echo $row_rsDICAS['DIC_TITULO']; ?>"/></div><div class="fix"></div></div>
                        
                        
                        <div class="rowElem nobg">
                        <label>Categoria</label>
                        <div class="formRight">
                        <select name="DICT_COD" id="DICT_COD" >
                          <?php
do {  
?>
                          <option value="<?php echo $row_rsDICAS_TIPO['DICT_COD']?>"<?php if (!(strcmp($row_rsDICAS_TIPO['DICT_COD'], $row_rsDICAS['DICT_COD']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsDICAS_TIPO['DICT_NOME']?></option>
                          <?php
} while ($row_rsDICAS_TIPO = mysql_fetch_assoc($rsDICAS_TIPO));
  $rows = mysql_num_rows($rsDICAS_TIPO);
  if($rows > 0) {
      mysql_data_seek($rsDICAS_TIPO, 0);
	  $row_rsDICAS_TIPO = mysql_fetch_assoc($rsDICAS_TIPO);
  }
?>
                        </select>
                    ...
                    <div class="rowElem nobg">
                        <label>Status</label>
                        <div class="formRight">
                        <select name="DIC_STATUS" id="DIC_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsDICAS['DIC_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsDICAS['DIC_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem">
                        ...
                        <div class="formRight moreFields onlyNums">
                            <ul>
                                <li><input type="text" name="DIC_DATA_DIA" maxlength="2" value=<?php echo $row_rsDICAS['DIC_DATA_DIA']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="DIC_DATA_MES" maxlength="2" value=<?php echo $row_rsDICAS['DIC_DATA_MES']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="DIC_DATA_ANO" maxlength="4" value=<?php echo $row_rsDICAS['DIC_DATA_ANO']; ?> /></li>
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsDICAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsDICAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsDICAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsDICAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                    ...
                    <div class="formRight"><input name="DIC_FOTO_LEGENDA" type="text" id="DIC_FOTO_LEGENDA" value="<?php echo $row_rsDICAS['DIC_FOTO_LEGENDA']; ?>" /></div><div class="fix"></div></div>
                  <div class="rowElem">
                  <label>Fonte:</label><div class="formRight"><input name="DIC_FONTE" type="text" id="DIC_FONTE" value="<?php echo $row_rsDICAS['DIC_FONTE']; ?>" /></div><div class="fix"></div></div>
                        <div class="rowElem">
                  <label>Tags:</label><div class="formRight"><input name="DIC_TAGS" type="text" id="DIC_TAGS" value="<?php echo $row_rsDICAS['DIC_TAGS']; ?>" /></div><div class="fix"></div></div>
              
                        <div class="rowElem">
                          <label>Resumo:</label><div class="formRight"><textarea name="DIC_CORPO_HOME" cols="" rows="8" id="DIC_CORPO_HOME"><?php echo $row_rsDICAS['DIC_CORPO_HOME']; ?></textarea>
                  </div><div class="fix"></div></div>
                        <div class="rowElem">
                        <label>Corpo:</label><div class="formRight"><textarea name="DIC_CORPO" cols="" rows="5" class="wysiwyg" id="DIC_CORPO"><?php echo $row_rsDICAS['DIC_CORPO']; ?></textarea>
                        </div><div class="fix"></div></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsDICAS_TIPO);
