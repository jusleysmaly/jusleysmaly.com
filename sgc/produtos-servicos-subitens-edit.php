...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_produtos_servicos_subitens SET PRDS_NOME=%s, PRDS_DESCRICAO=%s, PRD_COD=%s, PRDS_STATUS=%s, POS_COD_HOME=%s WHERE PRDS_COD=%s",
                       GetSQLValueString($_POST['PRDS_NOME'], "text"),
                       GetSQLValueString($_POST['PRDS_DESCRICAO'], "text"),
                       GetSQLValueString($_POST['PRD_COD'], "int"),
                       GetSQLValueString($_POST['PRDS_STATUS'], "int"),
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['PRDS_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "produtos-servicos-subitens.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsProdutosServicosTipos = "SELECT * FROM vw_produtos_servicos_tipo WHERE PRD_STATUS = 1 ORDER BY PRDT_COD ASC";
$rsProdutosServicosTipos = mysql_query($query_rsProdutosServicosTipos, $conn92ID) or die(mysql_error());
$row_rsProdutosServicosTipos = mysql_fetch_assoc($rsProdutosServicosTipos);
$totalRows_rsProdutosServicosTipos = mysql_num_rows($rsProdutosServicosTipos);

$colname_rsSubItens = "-1";
if (isset($_GET['URL_PRDS_COD'])) {
  $colname_rsSubItens = $_GET['URL_PRDS_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsSubItens = sprintf("SELECT * FROM tb_produtos_servicos_subitens WHERE PRDS_COD = %s", GetSQLValueString($colname_rsSubItens, "int"));
$rsSubItens = mysql_query($query_rsSubItens, $conn92ID) or die(mysql_error());
$row_rsSubItens = mysql_fetch_assoc($rsSubItens);
$totalRows_rsSubItens = mysql_num_rows($rsSubItens);
...
<title>Editar &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
...
<div class="wrapper">
    ...
    <div class="content" id="container">
    	<div class="title">
    	  <h5>Subitens &laquo; Produtos e Servi&ccedil;os &laquo; Categorias &laquo; Editar</h5>
    	</div>
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Nome:</label>
                  <input name="PRDS_COD" type="hidden" id="PRDS_COD" value="<?php echo $row_rsSubItens['PRDS_COD']; ?>" />
                  <div class="formRight"><input name="PRDS_NOME" type="text" id="PRDS_NOME" value="<?php echo $row_rsSubItens['PRDS_NOME']; ?>"/></div><div class="fix"></div></div>
                  
                  <div class="rowElem">
                        <label>Status</label>
                        <div class="formRight">
                        <select name="PRDS_STATUS" id="PRDS_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsSubItens['PRDS_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsSubItens['PRDS_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                  </div>
                  ...
                  <div class="rowElem nobg">
                      <label>Posição / Home</label>
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsSubItens['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsSubItens['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
...
do {  
?>
                          <option value="<?php echo $row_rsProdutosServicosTipos['PRD_COD']?>"<?php if (!(strcmp($row_rsProdutosServicosTipos['PRD_COD'], $row_rsSubItens['PRD_COD']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsProdutosServicosTipos['PRD_TITULO']?></option>
                          <?php
} while ($row_rsProdutosServicosTipos = mysql_fetch_assoc($rsProdutosServicosTipos));
  $rows = mysql_num_rows($rsProdutosServicosTipos);
  if($rows > 0) {
      mysql_data_seek($rsProdutosServicosTipos, 0);
	  $row_rsProdutosServicosTipos = mysql_fetch_assoc($rsProdutosServicosTipos);
  }
?>
                        </select>
                  ...
                  <div class="rowElem">
                  <label>Descri&ccedil;&atilde;o:</label><div class="formRight"><textarea name="PRDS_DESCRICAO" cols="" rows="8" id="PRDS_DESCRICAO"><?php echo $row_rsSubItens['PRDS_DESCRICAO']; ?></textarea></div><div class="fix"></div></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsProdutosServicosTipos);

mysql_free_result($rsSubItens);
?>
