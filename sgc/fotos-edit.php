...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_album SET POS_COD_HOME=%s, POS_COD_CANAL=%s, ALB_TITULO=%s, ALB_RESUMO=%s, ALB_DESCRICAO=%s, ALB_PICASA_USER=%s, ALB_PICASA_ALBUM=%s, ALB_DATA_DIA=%s, ALB_DATA_MES=%s, ALB_DATA_ANO=%s, ALB_STATUS=%s, ALB_TAGS=%s WHERE ALB_COD=%s",
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['ALB_TITULO'], "text"),
                       GetSQLValueString($_POST['ALB_RESUMO'], "text"),
                       GetSQLValueString($_POST['ALB_DESCRICAO'], "text"),
                       GetSQLValueString($_POST['ALB_PICASA_USER'], "text"),
                       GetSQLValueString($_POST['ALB_PICASA_ALBUM'], "text"),
                       GetSQLValueString($_POST['ALB_DATA_DIA'], "int"),
                       GetSQLValueString($_POST['ALB_DATA_MES'], "int"),
                       GetSQLValueString($_POST['ALB_DATA_ANO'], "int"),
                       GetSQLValueString($_POST['ALB_STATUS'], "int"),
                       GetSQLValueString($_POST['ALB_TAGS'], "text"),
                       GetSQLValueString($_POST['ALB_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "/sgc/fotos.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsALBUM = "-1";
if (isset($_GET['URL_ALB_COD'])) {
  $colname_rsALBUM = $_GET['URL_ALB_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsALBUM = sprintf("SELECT * FROM tb_album WHERE ALB_COD = %s", GetSQLValueString($colname_rsALBUM, "int"));
$rsALBUM = mysql_query($query_rsALBUM, $conn92ID) or die(mysql_error());
...
<title>Editar &laquo; &Aacute;lbum &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="ALB_COD" type="hidden" id="ALB_COD" value="<?php echo $row_rsALBUM['ALB_COD']; ?>" />
                  <div class="formRight"><input name="ALB_TITULO" type="text" id="ALB_TITULO" value="<?php echo $row_rsALBUM['ALB_TITULO']; ?>"/></div><div class="fix"></div></div>
                        
                        
                        <div class="rowElem nobg">
                        <label>Status</label>
                        <div class="formRight">
                        <select name="ALB_STATUS" id="ALB_STATUS" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsALBUM['ALB_STATUS']))) {echo "selected=\"selected\"";} ?>>0 - Inativa</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsALBUM['ALB_STATUS']))) {echo "selected=\"selected\"";} ?>>1 - Ativa</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem">
                        ...
                        <div class="formRight moreFields onlyNums">
                            <ul>
                                <li><input type="text" name="ALB_DATA_DIA" maxlength="2" value=<?php echo $row_rsALBUM['ALB_DATA_DIA']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="ALB_DATA_MES" maxlength="2" value=<?php echo $row_rsALBUM['ALB_DATA_MES']; ?> /></li>
                                <li class="sep">/</li>
                                <li><input type="text" name="ALB_DATA_ANO" maxlength="4" value=<?php echo $row_rsALBUM['ALB_DATA_ANO']; ?> /></li>
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsALBUM['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsALBUM['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" <?php if (!(strcmp(0, $row_rsALBUM['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsALBUM['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                  ...
                  <div class="rowElem">
                    <label>Picasa User:</label>
                    <div class="formRight"><input name="ALB_PICASA_USER" type="text" id="ALB_PICASA_USER"  value="<?php echo $row_rsALBUM['ALB_PICASA_USER']; ?>"/></div><div class="fix"></div></div>
                  <div class="rowElem">
                  <label>Picasa Album:</label><div class="formRight"><input name="ALB_PICASA_ALBUM" type="text" id="ALB_PICASA_ALBUM" value="<?php echo $row_rsALBUM['ALB_PICASA_ALBUM']; ?>" /></div><div class="fix"></div></div>
                        <div class="rowElem">
                  <label>Tags:</label><div class="formRight"><input name="ALB_TAGS" type="text" id="ALB_TAGS" /></div><div class="fix"></div></div>
              
                        <div class="rowElem">
                          <label>Resumo:</label><div class="formRight"><textarea name="ALB_RESUMO" cols="" rows="8" id="ALB_RESUMO"><?php echo $row_rsALBUM['ALB_RESUMO']; ?></textarea>
                  </div><div class="fix"></div></div>
                        <div class="rowElem">
                        <label>Descri&ccedil;&atilde;o:</label>
                        <div class="formRight"><textarea name="ALB_DESCRICAO" cols="" rows="5" class="wysiwyg" id="ALB_DESCRICAO"><?php echo $row_rsALBUM['ALB_DESCRICAO']; ?></textarea>
                        </div><div class="fix"></div></div>
