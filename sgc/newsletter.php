...
$colname_rsFRASES = "1";
if (isset($_GET['URL_NEW_STATUS'])) {
  $colname_rsFRASES = $_GET['URL_NEW_STATUS'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsFRASES = sprintf("SELECT * FROM tb_newsletter WHERE NEW_STATUS = %s ORDER BY new_cod desc", GetSQLValueString($colname_rsFRASES, "int"));
$rsFRASES = mysql_query($query_rsFRASES, $conn92ID) or die(mysql_error());
$row_rsFRASES = mysql_fetch_assoc($rsFRASES);
$totalRows_rsFRASES = mysql_num_rows($rsFRASES);

mysql_select_db($database_conn92ID, $conn92ID);
$query_rsCOUNT_ACTIVE = "SELECT COUNT(*) AS TOT_ACTIVE FROM tb_newsletter where new_status = 1";
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsCOUNT_INACTIVE = "SELECT COUNT(*) AS TOT_INACTIVE FROM tb_newsletter WHERE new_status = 0";
...
<div id="header" class="wrapper">
    ...
    <div class="middleNav">
    	<ul>
        	...
        	<li class="iOrders"><a href="newsletter-ins.php" title=""><span>Inserir Novo</span></a></li>
			<!-- InstanceEndEditable -->
        </ul>
    </div>
    <div class="fix"></div>
...
<div class="wrapper">
    ...
    <div class="content">
      ...
      <div class="stats">
        <ul>
          <li><a href="newsletter.php" class="count green" title=""><?php echo $row_rsCOUNT_ACTIVE['TOT_ACTIVE']; ?></a><span>Registros Ativos</span></li>
          <li class="last"><a href="depoimentos.php?URL_NEW_STATUS=0" class="count red" title=""><?php echo $row_rsCOUNT_INACTIVE['TOT_INACTIVE']; ?></a><span>Registros Inativos</span></li>
        </ul>
        <div class="fix"></div>
      </div>
      <!-- Static table -->
     ...
     <div class="widget">
        ...
        <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
          <thead>
            <tr>
              <th>T&iacute;tulo da Newsletter</th>
              <th>Imagem</th>
          ...
          <tbody>
            ...
            <!-- imagem -->
              <td><a href="newsletter-edit.php?URL_NEW_COD=<?php echo $row_rsFRASES['NEW_COD']; ?>"><?php echo $row_rsFRASES['NEW_COD']; ?> - <?php echo $row_rsFRASES['NEW_TITULO']; ?></a></td>
              <td class="center">
                <a href="/sgc/upload-single.php?URL_FILENAME=<?php echo $row_rsFRASES['NEW_COD']; ?>&amp;URL_DIR=newsletter&amp;IMG_SIZE=560 x 560 pixel [JPG]&amp;INAME=<?php echo $row_rsFRASES['NEW_TITULO']; ?>" target="_blank"><img src="images/icons/color/arrow-curve-090.png" alt="" class="mr10" /> </a>
                
                <?php If (file_exists('../images/content/newsletter/'.$row_rsFRASES['NEW_COD'].'.jpg')) { ?> <a href="/images/content/newsletter/<?php echo $row_rsFRASES['NEW_COD']; ?>.jpg" class="group1" ><img src="images/icons/color/image-resize-actual.png" alt="" class="mr10" /></a>                <?php } ?>
                <!-- /imagem -->
              ...
              <!-- ads -->
              <td class="center">
                <a href="/sgc/upload-single.php?URL_FILENAME=<?php echo $row_rsFRASES['NEW_COD']; ?>_ads&amp;URL_DIR=newsletter&amp;IMG_SIZE=520 x 200 pixel [JPG]&amp;INAME=<?php echo $row_rsFRASES['NEW_TITULO']; ?>" target="_blank"><img src="images/icons/color/arrow-curve-090.png" alt="" class="mr10" /> </a>
                
                <?php If (file_exists('../images/content/newsletter/'.$row_rsFRASES['NEW_COD'].'_ads.jpg')) { ?> <a href="/images/content/newsletter/<?php echo $row_rsFRASES['NEW_COD']; ?>_ads.jpg" class="group1" ><img src="images/icons/color/image-resize-actual.png" alt="" class="mr10" /></a>                <?php } ?>
              </td>
              <!-- /ads -->
              
              
              
              <td class="center"><a href="<?php echo $row_rsFRASES['NEW_CLIENTE']; ?>"  target="_blank" ><img src="images/icons/color/chain.png" alt="" class="mr10" /></a></td>
              <td class="center"><a href="/newsletter/index.php?URL_NEW_COD=<?php echo $row_rsFRASES['NEW_COD']; ?>"  target="_blank" ><img src="images/icons/color/mail.png" alt="" class="mr10" /></a></td>
              <td><span class="center">
              <?php $url_dis_status = isset( $_GET['URL_NEW_STATUS'] ) ? $_GET['URL_NEW_STATUS'] : null; ?>
              <?php
              switch ($url_dis_status) { 
			  case '0':
			  ?>
              <!-- ativa -->
              <a href="../_sgc_templates/active.php?URL_CHAVE=<?php echo $row_rsFRASES['NEW_COD']; ?>&amp;URL_REDIR=frases.php&amp;URL_TABLE=tb_newsletter&amp;URL_PREFIX=NEW_"><img src="images/icons/color/tick.png" alt="" class="mr10" /></a>
              <?php 
			break;
			default: 
			?>
              <!-- desativa -->
              <a href="inactive.php?URL_CHAVE=<?php echo $row_rsFRASES['NEW_COD']; ?>&amp;URL_REDIR=frases.php&amp;URL_TABLE=tb_newsletter&amp;URL_PREFIX=NEW_"><img src="images/icons/color/cross.png" alt="" class="mr10" /></a>
              <?php 
			  break;
			  } ?>
              </span></td>
            </tr>
              <?php } while ($row_rsFRASES = mysql_fetch_assoc($rsFRASES)); ?>
          </tbody>
        </table>
      </div>
