...
$query_rsCOUNT_ACTIVE = "SELECT COUNT(*) AS TOT_ACTIVE FROM tb_portfolio WHERE por_status = 1";
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsCOUNT_INACTIVE = "SELECT COUNT(*) AS TOT_INACTIVE FROM tb_portfolio WHERE por_status = 0";
...
$maxRows_rsPortfolio = 50;
$pageNum_rsPortfolio = 0;
if (isset($_GET['pageNum_rsPortfolio'])) {
  $pageNum_rsPortfolio = $_GET['pageNum_rsPortfolio'];
}
$startRow_rsPortfolio = $pageNum_rsPortfolio * $maxRows_rsPortfolio;

$colname_rsPortfolio = "1";
if (isset($_GET['URL_POR_STATUS'])) {
  $colname_rsPortfolio = $_GET['URL_POR_STATUS'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsPortfolio = sprintf("SELECT * FROM vc_portfolio_clientes_categorias WHERE POR_STATUS = %s", GetSQLValueString($colname_rsPortfolio, "int"));
$query_limit_rsPortfolio = sprintf("%s LIMIT %d, %d", $query_rsPortfolio, $startRow_rsPortfolio, $maxRows_rsPortfolio);
$rsPortfolio = mysql_query($query_limit_rsPortfolio, $conn92ID) or die(mysql_error());
$row_rsPortfolio = mysql_fetch_assoc($rsPortfolio);

if (isset($_GET['totalRows_rsPortfolio'])) {
  $totalRows_rsPortfolio = $_GET['totalRows_rsPortfolio'];
} else {
  $all_rsPortfolio = mysql_query($query_rsPortfolio);
  $totalRows_rsPortfolio = mysql_num_rows($all_rsPortfolio);
}
$totalPages_rsPortfolio = ceil($totalRows_rsPortfolio/$maxRows_rsPortfolio)-1;

$queryString_rsPortfolio = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsPortfolio") == false && 
        stristr($param, "totalRows_rsPortfolio") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsPortfolio = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsPortfolio = sprintf("&totalRows_rsPortfolio=%d%s", $totalRows_rsPortfolio, $queryString_rsPortfolio);
...
<title>Portfolio &laquo; 92ID:CMS &bull; &copy;92dpi</title>
<!-- InstanceEndEditable -->
...
<link href="css/colorbox.css" rel="stylesheet" type="text/css" />
		...
		<script>
			$(document).ready(function(){
				...
				$(".vimeo").colorbox({iframe:true, innerWidth:500, innerHeight:500});
...
<div id="header" class="wrapper">
    ...
    <div class="middleNav">
    	<ul>
        	...
        	<li class="iOrders"><a href="portfolio-ins.php?urlactive=02" title="Inserir Novo"><span>Inserir Novo</span></a></li>
			<!-- InstanceEndEditable -->
        </ul>
    </div>
    <div class="fix"></div>
...
<div class="wrapper">
    ...
    <div class="content">
      ...
      <div class="stats">
        <ul>
          <li><a href="produtos-servicos.php" class="count green" title=""><?php echo $row_rsCOUNT_ACTIVE['TOT_ACTIVE']; ?></a><span>Registros Ativos</span></li>
          <li class="last"><a href="produtos-servicos.php?URL_PRD_STATUS=0" class="count red" title=""><?php echo $row_rsCOUNT_INACTIVE['TOT_INACTIVE']; ?></a><span>Registros Inativos</span></li>
        </ul>
        <div class="fix"></div>
      </div>
      <!-- Static table -->
      ...
      <div class="widget">
        ...
        <table cellpadding="0" cellspacing="0" width="100%" class="tableStatic">
          ...
          <tbody>
            <?php do { ?>
            <tr class="gradeX">
              <td><a href="portfolio-edit.php?URL_POR_COD=<?php echo $row_rsPortfolio['POR_COD']; ?>"><?php echo $row_rsPortfolio['POR_TITULO']; ?></a></td>
              <td><?php echo $row_rsPortfolio['CLI_NOME']; ?></td>
              <td><?php echo $row_rsPortfolio['PRD_TITULO']; ?></td>
              <td class="center" ><?php echo $row_rsPortfolio['POS_COD_HOME']; ?></td>
              <!-- cover -->
              <td class="center"><a href="/sgc/upload-single.php?URL_FILENAME=<?php echo $row_rsPortfolio['POR_COD']; ?>-cover&amp;URL_DIR=portfolio&amp;IMG_SIZE= 800 x 800 pixel [JPG]" class="iframe"><img src="images/icons/color/arrow-curve-090.png" alt="" class="mr10" /></a>
                <?php If (file_exists('../images/content/portfolio/'.$row_rsPortfolio['POR_COD'].'-cover.jpg')) { ?>
                <a href="/images/content/portfolio/<?php echo $row_rsPortfolio['POR_COD']; ?>-cover.jpg" class="group1" ><img src="images/icons/color/image-resize-actual.png" alt="" class="mr10" /></a>
                <?php } ?></td>
              <!-- end:cover -->
              <!-- slider -->
              <td class="center"><a href="/sgc/upload-multi.php?URL_FILENAME=<?php echo $row_rsPortfolio['POR_COD']; ?>&amp;URL_DIR=portfolio&amp;IMG_SIZE=2000 x 800 pixel [JPG]" class="iframe"><img src="images/icons/color/arrow-curve-090.png" alt="" class="mr10" /></a>
                <?php If (file_exists('../images/content/portfolio/'.$row_rsPortfolio['POR_COD'].'-1.jpg')) { ?>
                <a href="/images/content/portfolio/<?php echo $row_rsPortfolio['POR_COD']; ?>-1.jpg" class="group1" ><img src="images/icons/color/image-resize-actual.png" alt="" class="mr10" /></a>
                <?php } ?></td>
              <!-- end:slider -->
              <td class="center"><?php $url_dis_status = isset( $_GET['URL_PRD_STATUS'] ) ? $_GET['URL_PRD_STATUS'] : null; ?>
                <?php
              switch ($url_dis_status) { 
			  case '0':
			  ?>
                <!-- ativa -->
                <a href="../_sgc_templates/active.php?URL_CHAVE=<?php echo $row_rsPortfolio['POR_COD']; ?>&amp;URL_REDIR=produtos-servicos.php&amp;URL_TABLE=tb_portfolio&amp;URL_PREFIX=PRD_"><img src="images/icons/color/tick.png" alt="" class="mr10" /></a>
                <?php 
			break;
			default: 
			?>
                <!-- desativa -->
                <a href="inactive.php?URL_CHAVE=<?php echo $row_rsPortfolio['POR_COD']; ?>&amp;URL_REDIR=produtos-servicos.php&amp;URL_TABLE=tb_portfolio&amp;URL_PREFIX=PRD_"><img src="images/icons/color/cross.png" alt="" class="mr10" /></a>
                <?php 
			  break;
			  } ?></td>
            </tr>
              <?php } while ($row_rsPortfolio = mysql_fetch_assoc($rsPortfolio)); ?>
          </tbody>
        </table>
      </div>
      ...
      <!-- Pagination -->
		<div class="pagination">
			<ul class="pages">
				<li class="prev"><a href="<?php printf("%s?pageNum_rsPortfolio=%d%s", $currentPage, max(0, $pageNum_rsPortfolio - 1), $queryString_rsPortfolio); ?>">&lt;</a></li>
				
				<li class="next"><a href="<?php printf("%s?pageNum_rsPortfolio=%d%s", $currentPage, min($totalPages_rsPortfolio, $pageNum_rsPortfolio + 1), $queryString_rsPortfolio); ?>">&gt;</a></li>
			</ul>
		</div>
