...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE tb_midias SET MID_TITULO=%s, MID_DESC=%s, POS_COD_HOME=%s, POS_COD_CANAL=%s, MID_STATUS=%s, MID_CODE_01=%s, MID_CODE_02=%s, MID_CODE_03=%s, MID_TAGS=%s, MIDT_COD=%s WHERE MID_COD=%s",
                       GetSQLValueString($_POST['MID_TITULO'], "text"),
                       GetSQLValueString($_POST['MID_DESC'], "text"),
                       GetSQLValueString($_POST['POS_COD_HOME'], "int"),
                       GetSQLValueString($_POST['POS_COD_CANAL'], "int"),
                       GetSQLValueString($_POST['MID_STATUS'], "int"),
                       GetSQLValueString($_POST['MID_CODE_01'], "text"),
                       GetSQLValueString($_POST['MID_CODE_02'], "text"),
                       GetSQLValueString($_POST['MID_CODE_03'], "text"),
                       GetSQLValueString($_POST['MID_TAGS'], "text"),
                       GetSQLValueString($_POST['MIDT_COD'], "int"),
                       GetSQLValueString($_POST['MID_COD'], "int"));

  mysql_select_db($database_conn92ID, $conn92ID);
  $Result1 = mysql_query($updateSQL, $conn92ID) or die(mysql_error());

  $updateGoTo = "/sgc/midias.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsMIDIAS_TIPO = "SELECT * FROM tb_midias_tipo ORDER BY MIDT_NOME ASC";
$rsMIDIAS_TIPO = mysql_query($query_rsMIDIAS_TIPO, $conn92ID) or die(mysql_error());
$row_rsMIDIAS_TIPO = mysql_fetch_assoc($rsMIDIAS_TIPO);
$totalRows_rsMIDIAS_TIPO = mysql_num_rows($rsMIDIAS_TIPO);

$colname_rsMIDIAS = "-1";
if (isset($_GET['URL_MID_COD'])) {
  $colname_rsMIDIAS = $_GET['URL_MID_COD'];
}
mysql_select_db($database_conn92ID, $conn92ID);
$query_rsMIDIAS = sprintf("SELECT * FROM tb_midias WHERE MID_COD = %s", GetSQLValueString($colname_rsMIDIAS, "int"));
$rsMIDIAS = mysql_query($query_rsMIDIAS, $conn92ID) or die(mysql_error());
$row_rsMIDIAS = mysql_fetch_assoc($rsMIDIAS);
$totalRows_rsMIDIAS = mysql_num_rows($rsMIDIAS);
...
<title>Inserir &laquo; M&iacute;dias &laquo; 92ID:CMS &laquo; &copy;92dpi</title>
<!-- InstanceEndEditable -->
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form" class="mainForm">
            ...
            <fieldset>
                <div class="widget first">
                  ...
                  <label>Título:</label>
                  <input name="MID_COD" type="hidden" id="MID_COD" value="<?php echo $row_rsMIDIAS['MID_COD']; ?>" />
                  <div class="formRight"><input name="MID_TITULO" type="text" id="MID_TITULO" value="<?php echo $row_rsMIDIAS['MID_TITULO']; ?>"/></div><div class="fix"></div></div>
                        
                        
                        <div class="rowElem nobg">
                        <label>Tipo</label>
                        <div class="formRight">
                        <select name="MIDT_COD" id="MIDT_COD" >
                          <?php
do {  
?>
                          <option value="<?php echo $row_rsMIDIAS_TIPO['MIDT_COD']?>"<?php if (!(strcmp($row_rsMIDIAS_TIPO['MIDT_COD'], $row_rsMIDIAS['MIDT_COD']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsMIDIAS_TIPO['MIDT_NOME']?></option>
                          <?php
} while ($row_rsMIDIAS_TIPO = mysql_fetch_assoc($rsMIDIAS_TIPO));
  $rows = mysql_num_rows($rsMIDIAS_TIPO);
  if($rows > 0) {
      mysql_data_seek($rsMIDIAS_TIPO, 0);
	  $row_rsMIDIAS_TIPO = mysql_fetch_assoc($rsMIDIAS_TIPO);
  }
?>
                        </select>
                    ...
                    <div class="rowElem nobg">
                        <label>Status</label>
                        <div class="formRight">
                        <select name="MID_STATUS" id="MID_STATUS" >
                  ...
                  <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_HOME" id="POS_COD_HOME" >
                          <option value="0" selected="selected" <?php if (!(strcmp(0, $row_rsMIDIAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsMIDIAS['POS_COD_HOME']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    ...
                    <div class="rowElem nobg">
                        ...
                        <select name="POS_COD_CANAL" id="POS_COD_CANAL" >
                          <option value="0" selected="selected" <?php if (!(strcmp(0, $row_rsMIDIAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>0 - Não Entra</option>
                          <option value="1" <?php if (!(strcmp(1, $row_rsMIDIAS['POS_COD_CANAL']))) {echo "selected=\"selected\"";} ?>>1 - Entra</option>
                        </select>
                        </div>
                        <div class="fix"></div>
                    </div>
                    <div class="rowElem">
                  <label>Tags:</label><div class="formRight"><input name="MID_TAGS" type="text" id="MID_TAGS" value="<?php echo $row_rsMIDIAS['MID_TAGS']; ?>" /></div><div class="fix"></div></div>
              
              <div class="rowElem">
                          <label>Descri&ccedil;&atilde;o:</label><div class="formRight"><textarea name="MID_DESC" cols="" rows="8" id="MID_DESC"><?php echo $row_rsMIDIAS['MID_DESC']; ?></textarea></div><div class="fix"></div></div>
                          
                        <div class="rowElem">
                          <label>Code#01 (Home):</label><div class="formRight"><textarea name="MID_CODE_01" cols="" rows="8" id="MID_CODE_01"><?php echo $row_rsMIDIAS['MID_CODE_01']; ?></textarea></div><div class="fix"></div></div>
                          
                          <div class="rowElem">
                  <label>Code#02 (Canal):</label><div class="formRight"><textarea name="MID_CODE_02" cols="" rows="8" id="MID_CODE_02"><?php echo $row_rsMIDIAS['MID_CODE_02']; ?></textarea></div><div class="fix"></div></div>
                          
                          <div class="rowElem">
                  <label>Code#03 (Outros):</label><div class="formRight"><textarea name="MID_CODE_03" cols="" rows="8" id="MID_CODE_03"><?php echo $row_rsMIDIAS['MID_CODE_03']; ?></textarea></div><div class="fix"></div></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsMIDIAS_TIPO);

mysql_free_result($rsMIDIAS);
?>
